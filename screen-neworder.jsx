/* New Order modal — choose client + order type */

const { useState: useStateN } = React;

function NewOrderModal({ onClose, onContinue }) {
  const [step, setStep] = useStateN(1);
  const [clientId, setClientId] = useStateN("");
  const [type, setType] = useStateN("hospital");
  const [q, setQ] = useStateN("");

  const clients = window.HOSPITAL_CLIENTS.filter(c =>
    ...
    <Modal onClose={onClose} size="lg">
      <div className="modal__head">
        ...
        <div className="stepper" style={{marginTop: 18}}>
          <div className={"stepper__item " + (step === 1 ? "is-active" : "is-done")}><span className="stepper__num">{step > 1 ? "✓" : "1"}</span> Select client</div>
          <div className="stepper__sep"/>
          <div className={"stepper__item " + (step === 2 ? "is-active" : (step > 2 ? "is-done" : ""))}><span className="stepper__num">2</span> Order type</div>
          <div className="stepper__sep"/>
          <div className="stepper__item"><span className="stepper__num">3</span> Build order</div>
        </div>
      </div>
          ...
          <>
            <div className="field" style={{marginBottom: 14}}>
              ...
              <div className="search">
                ...
                <input autoFocus className="input" placeholder="Search by hospital name or account code…" value={q} onChange={e => setQ(e.target.value)} />
              </div>
            </div>
            <div style={{maxHeight: 320, overflow: "auto", border: "1px solid var(--border)", borderRadius: 8}}>
              {clients.map(c => (
                <div key={c.id}
                     ...
                     style={{padding: "12px 14px", cursor: "pointer", borderBottom: "1px solid var(--border)", background: clientId === c.id ? "#f6f6f3" : "transparent"}}>
                  ...
                  <div style={{flex: 1, minWidth: 0}}>
                    ...
                    <div className="muted mono" style={{fontSize: 11.5, marginTop: 2}}>{c.code} · {c.region} · {c.group}</div>
                  </div>
                  <div className="muted" style={{fontSize: 12}}>Terms: {c.terms}</div>
                  {clientId === c.id && <Icon name="check" size={18} style={{color: "var(--ink)"}}/>}
                </div>
              ))}
              {clients.length === 0 && <div className="empty">No clients match “{q}”.</div>}
            </div>
          </>
        )}
      ...
      <div className="modal__foot">
        <div className="muted" style={{fontSize: 12.5}}>
          {step === 1 && (clientId ? <>Selected: <b style={{color:"var(--ink-2)"}}>{window.HOSPITAL_CLIENTS.find(c => c.id === clientId)?.name}</b></> : "Pick a hospital account to continue")}
          {step === 2 && <>Building a <b style={{color:"var(--ink-2)"}}>Hospital</b> order for <b style={{color:"var(--ink-2)"}}>{window.HOSPITAL_CLIENTS.find(c => c.id === clientId)?.name}</b></>}
        </div>
        <div className="row gap-8">
          ...
          {step === 1 && <button className="btn btn--primary" disabled={!clientId} onClick={() => setStep(2)}>Continue <Icon name="arrow-right" size={14}/></button>}
          {step === 2 && <button className="btn btn--primary" disabled={!canContinue} onClick={() => onContinue({ clientId, type })}>Start order <Icon name="arrow-right" size={14}/></button>}
        </div>
      </div>
    </Modal>
  );
